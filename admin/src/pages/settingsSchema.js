...
export const GROUP_FIELDS = {
  ...
  retention: [
    ...
    { name: 'v2_enabled', label: 'Retention v2 enabled', type: 'bool', section: 'v2', help: 'Switch this product from the v1 ping matrix to the event-driven agent loop. Exactly one proactive regime runs: v1 skips v2 products and vice versa.' },
    { name: 'v2_dry_run', label: 'v2 dry-run (shadow mode)', type: 'bool', section: 'v2', help: 'ON: the agent decides and logs to the Retention v2 ledger but sends nothing. Turn off only after reviewing decisions.' },
    { name: 'v2_daily_budget_usd', label: 'v2 daily AI budget (USD)', type: 'float', min: 0, max: 10000, section: 'v2', help: 'Hard stop: once the day’s decisions cost this much, the v2 loop goes quiet until tomorrow. 0 = no budget.' },
    { name: 'v2_loss_comfort_hours', label: 'v2 loss comfort window (hours)', type: 'int', min: 0, max: 720, section: 'v2', help: 'After a big-loss signal: no play invitations, no reward photos, empathetic tone only, for this many hours.' },
    { name: 'v2_loss_high_usd', label: 'v2 high-loss threshold (USD / 24h)', type: 'float', min: 0, max: 1000000, section: 'v2', help: 'Net loss over 24 hours that marks the player critical and starts the comfort window.' },
    { name: 'v2_same_event_cooldown_hours', label: 'v2 same-event cooldown (hours)', type: 'int', min: 0, max: 720, section: 'v2', help: 'One reaction per event TYPE per player per window — five deposits in an evening get one warm note, not five. Set 0 to disable while testing, so a re-injected simulator event gets a fresh decision instead of a same_event_cooldown block.' },
