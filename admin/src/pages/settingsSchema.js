...
export const GROUP_FIELDS = {
  antispam: [
    ...
    { name: 'partner_rate_limit_max', label: 'Partner event feed (max requests / window)', type: 'int', min: 1, max: 1000000, module: 'retention', help: 'Allowance for the casino’s server-to-server event feed, separate from the widget’s per-IP limit. Each request may carry 500 events; too low and the pipeline is throttled at its very first hop, before anything else can matter.' },
  ...
  retention: [
    ...
    { name: 'partner_rate_per_sec', label: 'Push/in-app: sends / sec (casino endpoint)', type: 'float', min: 0.1, max: 1000, step: 1, section: 'pipeline', help: 'Ceiling for delegated push and in-app, which are POSTs at the casino’s own delivery endpoint. Unshaped, a large broadcast is a denial of service against our partner — who then throttles or drops us, and those touches are lost on their side where we cannot retry them.' },
    { name: 'send_pass_max_sec', label: 'Max seconds per send pass', type: 'int', min: 1, max: 600, section: 'pipeline', globalOnly: true, help: 'How long one product may keep sending before handing the worker slot back. The queue is durable, so the next tick resumes where this stopped; the bound only stops one product’s burst from starving the others.' },    { name: 'event_keep_days', label: 'Keep events (days)', type: 'int', min: 1, max: 3650, section: 'pipeline', globalOnly: true, help: 'Age at which decision-worthy events are pruned. The log is append-only and grows by millions of rows a month; with no reaper the table and every query over it degrade until the drain itself slows down.' },
