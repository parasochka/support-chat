...
export const GROUP_FIELDS = {
  ...
  retention: [
    ...
    { name: 'media_normalize_interval_sec', label: 'Normalize sweep interval (sec)', type: 'int', min: 300, max: 86400, section: 'media', globalOnly: true, help: 'How often the media sweep runs — ONE loop serves every product, so this is a deploy-wide (global) setting. Default 3600 (hourly). The «Normalize now» button on the Media tab runs one product immediately.' },
    { name: 'media_max_side_px', label: 'Max photo side (px)', type: 'int', min: 512, max: 4096, section: 'media', help: 'Longest side after normalization. Telegram re-compresses photos to ~2560 px anyway, so 2048 keeps full delivered quality at a fraction of the size.' },
    { name: 'media_webp_quality', label: 'WebP quality (40–100)', type: 'int', min: 40, max: 100, section: 'media', help: 'Compression quality of the normalized WebP. 82 is visually lossless for chat photos; raise it only if you see artifacts.' },
