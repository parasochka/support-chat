...
export const GROUP_FIELDS = {
  ...
  model: [
    ...
    { name: 'deepseek_config', label: 'DeepSeek parameters (JSON)', type: 'json', tab: 'DeepSeek', help: 'All DeepSeek request parameters as one JSON object: "model" (deepseek-v4-flash / deepseek-v4-pro), "max_output_tokens" (sent as max_tokens), "base_url", the shared timeout/attempt knobs, plus any extra API params ("temperature", …) passed verbatim. Add a "pricing" block ({"input_per_1m", "cached_input_per_1m", "output_per_1m"}, USD per 1M tokens) so dashboard costs stay correct for models the built-in table doesn\'t know.' },
  ],
  general: [
