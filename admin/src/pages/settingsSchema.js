...
export const GROUP_FIELDS = {
  ...
  retention: [
    // --- Dialogue & photos (the reactive chat) ---------------------------
    { name: 'daily_photo_cap', label: 'Daily photo cap', type: 'int', min: 0, max: 10000, section: 'dialogue', help: 'Max photos sent to one player per day (hard, incl. requested).' },
    { name: 'proactive_photo_cooldown_msgs', label: 'Proactive photo cooldown (msgs)', type: 'int', min: 1, max: 10000, section: 'dialogue', help: 'Messages between UNPROMPTED photos (a direct ask bypasses it).' },
    { name: 'candidate_list_size', label: 'Photo candidate list size', type: 'int', min: 1, max: 50, section: 'dialogue', help: 'How many photo candidates the model is offered to choose from.' },
    { name: 'play_reminder_every_msgs', label: 'Play reminder every N replies', type: 'int', min: 0, max: 1000, section: 'dialogue', help: 'Every N-th of Nika’s Telegram replies weaves in a light in-context invitation to play, with a one-tap site button picked from the Site map by intent (0 = off).' },
    { name: 'session_idle_minutes', label: 'Session idle (min)', type: 'int', min: 0, max: 525600, section: 'dialogue', help: 'Idle minutes before a Telegram chat closes; the next message starts a fresh chat (0 = never close).' },
    { name: 'carry_context_turns', label: 'Carry-over context turns', type: 'int', min: 0, max: 50, section: 'dialogue', help: 'Trailing turns of the previous chat shown to the model when a returning player starts a fresh one (0 = off).' },
    // --- Proactive agent ---------------------------------------------------
    ...
    // --- Inactivity ladder (write-first to a silent player) ----------------
    { name: 'inactivity_enabled', label: 'Inactivity ladder enabled', type: 'bool', section: 'inactivity', help: 'Write FIRST to a player who went quiet: warm re-engagement touches at the ladder days below. Also obeys the agent switch, dry-run and every guard. RG-hold players are never touched.' },
    { name: 'inactivity_steps', label: 'Ladder steps (days idle)', type: 'intlist', section: 'inactivity', help: 'Days of inactivity that trigger a touch, one per line (spec ladder: 7 / 10 / 14 / 21 / 30). One step = at most one touch per idle spell; any player activity restarts the ladder.' },
    { name: 'v2_follow_up_hours', label: 'Follow-up after (hours)', type: 'int', min: 0, max: 720, section: 'inactivity', help: 'One gentle follow-up this many hours after an unanswered re-engagement touch (0 = off). Only the inactivity/VIP touches chain — event reactions never do.' },
    { name: 'v2_vip_at_risk_days', label: 'VIP at-risk after (days)', type: 'int', min: 0, max: 365, section: 'inactivity', help: 'A top-two-tier VIP idle this many days gets a dedicated warm touch and a durable admin event (visible in the System log, with the assigned manager). 0 = off.' },
    // --- Send-frequency guards ---------------------------------------------
    ...
    { name: 'v2_backoff_after_ignored', label: 'Back off after N ignored touches', type: 'int', min: 0, max: 50, section: 'guards', help: 'After this many consecutive proactive messages with no reply, the min-gap stretches 4× until the player writes again (0 = off). The channel quiets down instead of insisting.' },
    ...
    { name: 'quiet_hours_utc_offset', label: 'Quiet hours UTC offset', type: 'int', min: -12, max: 14, section: 'guards', help: 'Fallback timezone offset for quiet hours and the prompt’s current-time block (e.g. 3 = UTC+3). When the casino feed supplies a player’s own timezone, that player’s clock is used instead.' },
    ...
    { name: 'v2_loss_delay_min', label: 'Loss reaction delay (minutes)', type: 'int', min: 0, max: 1440, section: 'guards', help: 'Wait this long after the triggering bet before the comfort note, so it never lands mid-session (spec: 30–60 min). 0 = react immediately.' },
    // --- Photo unlock progression ------------------------------------------
    ...
    { name: 'stage_advance_min_hours', label: 'Stage advance min hours', type: 'int', min: 0, max: 8760, section: 'progression', help: 'Minimum spacing between explicitness-stage advances.' },
    ...
    // --- Plumbing ------------------------------------------------------------
    { name: 'nonce_ttl_sec', label: 'Deeplink nonce TTL (sec)', type: 'int', min: 10, max: 3600, section: 'plumbing', help: 'Lifetime of a one-time deeplink nonce.' },
    { name: 'profile_pull_ttl_sec', label: 'Profile pull TTL (sec)', type: 'int', min: 0, max: 604800, section: 'plumbing', help: 'How long a pulled player profile stays fresh before a re-pull.' },
  ],
};
