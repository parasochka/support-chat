...
`prompts.py`, …) — they live under the package paths (`app/core/db.py`,
`app/ai/prompts.py`, …; locate any of them with Glob).
...
### EVENT PIPELINE / OUTCOME ATTRIBUTION / ORCHESTRATOR — moved to the retention-bot skill
The durable leased event queue (`retention_v2.py`, `send_worker.py` — the claim/lease
lifecycle, priority lanes, backpressure, the send stage, process roles, the hot knobs),
outcome attribution (`app/retention/outcomes.py`) and the DOC-0..DOC-7 orchestrator
layer (measurement / RG guard / frequency / scoring / offers / journeys / channels)
are fully specified in the `retention-bot` skill
(`.claude/skills/retention-bot/SKILL.md`) — load it before touching any of those
modules. The non-negotiable queue rules stay resident here: Invariants 10-14 below.

### SPEND ATTRIBUTION + QUALITY REVIEW — moved to the admin-surface skill
Per-call spend labels (`ai_interaction_logs.consumer/.source`) and the LLM-as-judge
quality pass (`app/ai/reviewer.py`, the Common → Quality page) are specified in the
`admin-surface` skill (`.claude/skills/admin-surface/SKILL.md`) — load it before
touching dashboards, cost reads, or the reviewer. The resident rule is Invariant 4:
every AI log row carries `consumer` + `source`; readers go through
`db._LOG_SOURCE`/`_LOG_IS_*`, never re-deriving the spender from `session_id IS NULL`.
