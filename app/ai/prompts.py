...
def render_prompt_variables(text: str) -> str:
    ...
    """
    from app.core import settings  # lazy: prompts must stay importable without the app wired up
...
def get_system_core() -> str:
    ...
    from app.core import settings  # lazy: avoid an import cycle at module load
...
_RETENTION_ENGAGEMENT_DIRECTIVE = (
    ...
    "- Do NOT LATCH onto one word, image or motif. Once you have used a "
    "striking word or image (a whisper, the night, a scene, a running "
    "metaphor), it is SPENT for this conversation: do not carry it into your "
    "next messages, do not build reply after reply around it, and do not echo "
    "the same key word across consecutive messages - even when the player "
    "responds well to it. Riffing on one motif message after message "
    "(\"whisper... whispered... in a whisper\") is the loudest bot tell; each "
    "reply finds a genuinely different image or simply speaks plainly with no "
    "imagery at all.\n"
