...
from app.retention import retention as retention_mod
...
async def retention_effective_prompt(product_id: int,
    ...
    from app.api.admin import _preview_context  # reuse the Test-sandbox preview player
    await admin_auth.require_product_read(admin, product_id)
...
async def overview(product_id: Optional[int] = None,
                   ...
                   admin=Depends(require_admin)) -> JSONResponse:
    from app.api.admin import _range  # reuse the shared date-range parser
