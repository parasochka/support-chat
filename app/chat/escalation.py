...
Escalation comes in two strengths:
  - HARD (final=True): the model signalled it cannot help ([[ESCALATE]]), the
    session hit the HARD message ceiling (soft cap x HARD_CAP_MULTIPLIER), or
    the player tapped the explicit escalate action (incl. the cap-notice
    "hand over to a human" button). The session is closed (status='escalated')
    and the widget ends the conversation — only a new chat is possible after
    that.
...
# The message cap (`general.max_messages_per_session`) is SOFT: the turn that
# reaches it (and every turn after) still gets a real model answer, but the
# response carries the localized cap notice (cap_notice_payload) with two
# explicit actions - escalate to a human (the product's normal escalation flow,
# incl. the retention-bot deeplink) or finish the chat - instead of the
# suggestion bubbles. The old behaviour (force-escalate and close exactly at the
# cap, with no explanation) landed as a wall on live conversations. A hard
# ceiling remains as the cost backstop: at HARD_CAP_MULTIPLIER x the cap the
# session force-escalates and closes as before, so a runaway chat can never
# burn tokens without bound.
HARD_CAP_MULTIPLIER = 2


def hard_message_limit() -> int:
    """The absolute per-session message ceiling (cap x multiplier)."""
    from app.core import settings  # lazy: avoids a settings<->escalation cycle
    return int(settings.general()["max_messages_per_session"]) * HARD_CAP_MULTIPLIER


def cap_notice_payload(lang: str) -> dict:
    """The localized long-chat notice block for the /message response.

    Returned (as `cap_notice`) with every reply once the conversation has
    reached the soft message cap and is still open: a stable explanation that
    the chat has stalled plus the two button labels - escalate to a human
    (POST /escalate, the normal flow) and finish the chat (POST /resolve).
    The copy lives in the translations registry (admin Translations tab),
    never generated by the model.
    """
    from app.i18n import translations  # lazy: keep this module import-light
    return {
        "message": translations.text("cap_notice_message", lang),
        "escalate_label": translations.text("cap_escalate_button", lang),
        "finish_label": translations.text("cap_finish_button", lang),
    }
...
def _normalized(text: str) -> str:
    ...
    from app.chat import antispam  # lazy: avoids the settings/escalation import cycle
...
def keyword_trigger(user_text: str) -> Optional[str]:
    ...
    """
    from app.core import settings  # lazy import to avoid a settings<->escalation cycle
...
def decide(*, model_signalled: bool, message_count: int) -> EscalationDecision:
    ...
    `message_count` is the PROSPECTIVE count for this turn (current + 1). The
    SOFT cap (`max_messages_per_session`) no longer closes anything - past it the
    reply carries the cap notice (see cap_notice_payload) and the chat stays
    open. Only the HARD ceiling (cap x HARD_CAP_MULTIPLIER) force-escalates
    here, as the cost backstop. The model-free fast path in api/chat.py is the
    cheap belt-and-suspenders for a session already AT/over that ceiling (e.g.
    after the owner lowers the cap mid-session); the two are complementary, not
    a duplicate check.
    """
...
def build_payload(lang: str, final: bool = True) -> dict:
    ...
    """
    from app.core import settings      # lazy to keep this module import-light
    from app.core import tenancy       # lazy: same
    from app.i18n import translations  # lazy: same
...
async def build_payload_for_session(session: dict, lang: str,
    ...
    from app.core import db          # noqa: PLC0415
    from app.retention import retention   # noqa: PLC0415
    from app.core import tenancy     # noqa: PLC0415
    try:
...
async def apply_hard_escalation(session: dict, reason: str) -> None:
    ...
    """
    from app.core import db  # noqa: PLC0415 — lazy, mirrors build_payload_for_session
