...
| `RETENTION_MEDIA_NORMALIZE_ENABLED` | no | `true` | Default for `retention.media_normalize_enabled` — the periodic media-normalizer sweep: heavy JPG/PNG uploads are re-encoded to Telegram-sized WebP and the originals deleted (GIFs left alone). |
...
| `RETENTION_MEDIA_MAX_SIDE_PX` | no | `2048` | Default for `retention.media_max_side_px` — longest photo side after normalization (Telegram re-compresses photos to ~2560 px anyway, so 2048 keeps full delivered quality). |
| `RETENTION_MEDIA_WEBP_QUALITY` | no | `82` | Default for `retention.media_webp_quality` — WebP quality of the normalized photo (82 is visually lossless for chat photos). |
