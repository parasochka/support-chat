...
- **Proactive retention pings (Telegram)** — an admin-managed "ping matrix" per product
  (Retention → Pings): rules like "idle in the bot / on the casino / no deposit for N days →
  Nika reaches out", with the message AI-generated by the same retention persona (optionally
  with a gated photo). Hard anti-annoyance guards on top of the rules: per-player daily cap,
  minimum gap between pings, local quiet hours, per-rule cooldowns, a `/stop` opt-out
  (`/resume` to re-enable), and blocked-bot detection. Off by default; a product opts in via
  the `retention.pings_enabled` setting. Casino-activity triggers need the partner to feed
...
- **Admin dashboard** (`/admin` — the React Admin SPA in `admin/`, compiled by the
  ...
  in-range KPIs (engagement, photos, pings + reply rate, cost, stage distribution),
...
| `RETENTION_PING_DAILY_CAP` | no | `1` | Hard per-player cap: at most this many pings a day, no matter how many rules match. |
| `RETENTION_PING_MIN_GAP_HOURS` | no | `48` | Minimum gap between any two pings to the same player. |
